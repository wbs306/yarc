...
import {
  getIeeeSessionCookieJar,
  ieeeXploreService,
  readIeeeJournalBrowserPreferences,
} from './ieee-xplore.service.js'
...
const TRUSTED_PDF_HOSTS = new Set(['ieeexplore.ieee.org'])
...
const addResponseCookies = (headers: Headers, jar: Map<string, string>) => {
  for (const cookie of readSetCookieHeaders(headers)) {
    ...
    if (value && value !== '_remove_') jar.set(name, value)
...
async function assertSafePdfDownloadUrl(rawUrl: string): Promise<void> {
  ...
  // IEEE PDF URLs are generated by YARC itself. Some proxy/TUN DNS setups
  // resolve public domains to synthetic private IPv6 addresses, so DNS-based
  // SSRF checks would otherwise reject every IEEE preview before connecting.
  if (TRUSTED_PDF_HOSTS.has(hostname)) return
...
async function fetchWithCookieJar(
  ...
  for (let redirect = 0; redirect <= maxRedirects; redirect++) {
    ...
    response = transport === 'native'
      ? await fetch(currentUrl, { ...init, headers, redirect: 'manual' })
      : await fetchOverHttp1(currentUrl, { ...init, headers })
  ...
  return response || (transport === 'native'
    ? fetch(url, { ...init, redirect: 'manual' })
    : fetchOverHttp1(url, init))
}
...
export class SearchService {
  ...
  async fetchPdfStream(url: string, range?: string, method: 'GET' | 'HEAD' = 'GET'): Promise<Response> {
    const ieeeArticleNumber = extractIEEEArticleNumber(url)
    const downloadUrl = ieeePdfUrlForArticle(url) || url
    const jar = ieeeArticleNumber ? getIeeeSessionCookieJar() : new Map<string, string>()
    const requestPdf = (transport: 'http1' | 'native' = 'http1') => fetchWithCookieJar(downloadUrl, {
      method,
      headers: {
    ...
    // Reuse cookies captured while searching/browsing IEEE metadata. With an
    // established institution session, subscribed PDFs should go directly over
    // the native HTTP/2-capable transport without an extra abstract request.
    let response = await requestPdf(ieeeArticleNumber && jar.size ? 'native' : 'http1')
    let contentType = response.headers.get('content-type') || ''

    // IEEE occasionally returns an error page on the first direct PDF request
    // until the article endpoint has established the institution entitlement
    // cookies. Journal browsing usually does this implicitly; Markdown paper
    // references can be the first IEEE request in a fresh session.
    if (ieeeArticleNumber && (!response.ok || !contentType.toLowerCase().includes('pdf'))) {
      try { await response.body?.cancel() } catch {}
      // Discard cookies from the failed redirect chain. IEEE can bind that
      // session to its error page, making a retry with the same jar fail again.
      jar.clear()
      try {
        const entitlement = await fetchWithCookieJar(
          `${IEEE_BASE_URL}/rest/document/${ieeeArticleNumber}/abstract`,
          {
            headers: {
              Accept: 'application/json, text/plain, */*',
              Referer: `${IEEE_BASE_URL}/document/${ieeeArticleNumber}/`,
        ...
        try { await entitlement.body?.cancel() } catch {}
      } catch {
        // The retry below remains authoritative; preserve its status/error.
      }
      // Some subscribed IEEE documents serve their PDF only over the native
      // HTTP/2-capable transport; the HTTP/1.1 endpoint returns an HTML wrapper.
      response = await requestPdf('native')
