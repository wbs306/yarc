...
import { normalizeProjectSettings } from '../lib/project-settings.js'
...
export class ProjectService {
  ...
  async create(input: { name: string; directoryName?: string; description?: string; settings?: Record<string, unknown> }) {
    ...
    const settings = normalizeProjectSettings(input.settings === undefined ? {} : input.settings)
  ...
  async update(id: string, input: { name?: string; description?: string | null; settings?: Record<string, unknown> }) {
    ...
    if (input.settings !== undefined) data.settings = normalizeProjectSettings(input.settings)
  ...
  async delete(id: string) {
    ...
    const { projectLatexService } = await import('./project-latex.service.js')

    // Stop Project-scoped compilers before removing their source root. This
    // also settles build History metadata while the Project still exists.
    await projectLatexService.disposeProject(id)
