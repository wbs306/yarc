...
export class ProjectLatexService {
  ...
  async targets(projectId: string) {
    ...
    const targets = Array.isArray(latex.targets) ? latex.targets as ProjectLatexTarget[] : []
    return { defaultTarget: latex.defaultTarget || targets[0]?.id || null, targets }
  }
  ...
  private async target(projectId: string, targetId: string) {
    ...
    if (!['pdflatex', 'xelatex', 'lualatex'].includes(target.engine)) throw new AppError('VALIDATION_ERROR', 'Unsupported LaTeX engine', 400)
    return target
  }
  ...
  async startBuild(projectId: string, targetId: string) {
    ...
    if (!entry || extname(entry).toLowerCase() !== '.tex') throw new AppError('INVALID_LATEX_ENTRY', 'LaTeX entry must be a .tex file', 400)
