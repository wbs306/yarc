...
import { parseProjectChangePath, projectChangeScope } from '../lib/project-change-path.js'
...
export class ProjectWorkspaceWatcherService {
  ...
  async start() {
    ...
    const watcher = getDataChangeWatcher(projectChangeScope().root)
  ...
  private async onChange(change: DataChangeEvent) {
    ...
    const projectPath = parseProjectChangePath(path)
    if (!projectPath) return
    const { directoryName, relativePath } = projectPath
    ...
    if (!projectId) {
      ...
      this.directoryCache.set(directoryName, resolvedProjectId)
      await this.initializeTrackedPaths(resolvedProjectId)
    }
    if (!projectId) return
