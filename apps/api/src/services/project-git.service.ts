...
export class ProjectGitService {
  private async run(projectId: string, args: string[], options: { allowFailure?: boolean } = {}): Promise<RunResult & { code: number }> {
    ...
    return new Promise((resolve, reject) => {
      // API inputs are selected filesystem paths, never Git pathspec expressions.
      const child = spawn('git', ['--literal-pathspecs', ...args], { cwd: root, shell: false, stdio: ['ignore', 'pipe', 'pipe'] })
      let stdout = ''
      let stderr = ''
  ...
  async status(projectId: string): Promise<ProjectGitStatus> {
    const [branchResult, headResult, porcelain] = await Promise.all([
      ...
      this.run(projectId, ['status', '--porcelain=v1', '-z', '--untracked-files=all']),
    ])
  ...
  async stage(projectId: string, paths: string[]) {
    ...
    const result = await this.status(projectId)
    sseHub.emit({ type: 'project-files-changed', projectId, action: 'git-change', at: new Date().toISOString() })
    return result
  }
  ...
  async commit(projectId: string, message: string) {
    ...
    const result = { status: await this.status(projectId), commits: await this.log(projectId, 1) }
    sseHub.emit({ type: 'project-files-changed', projectId, action: 'git-change', at: new Date().toISOString() })
    return result
  }
  ...
  async createBranch(projectId: string, name: string) {
    ...
    const result = await this.branches(projectId)
    sseHub.emit({ type: 'project-files-changed', projectId, action: 'git-change', at: new Date().toISOString() })
    return result
  }
