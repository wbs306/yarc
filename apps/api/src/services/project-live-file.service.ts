  ...
  async get(projectId: string) {
    ...
    const raw = new LiveFileService({
      ...
      afterWrite: async (path, source) => {
        ...
        const change = projectChangePath(project.directoryName, path)
        await getDataChangeWatcher(change.root).publish({ path: change.path, source: 'live-file' }).catch(error => {
          console.warn('[ProjectLiveFile] failed to publish workspace change:', (error as Error).message)
        })
      },
    })
