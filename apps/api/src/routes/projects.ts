...
projects.get('/:id/files/office/view', async (c) => {
  const path = c.req.query('path') || ''
  ...
  const file = await projectFileService.getOfficeFile(id(c), path)
  return c.json({ mode, content: await runOfficeView(file.path, mode) })
})
...
projects.put('/:id/history/settings', async (c) => {
  ...
  // Finish pending writing-file revisions under the old policy. The settings
  // themselves are metadata and never become a Writing History revision.
...
projects.post('/:id/history/flush', async (c) => {
  const projectId = id(c)
  await projectLiveFileManager.flushProject(projectId)
  return c.json({ checkpoint: await projectHistoryService.flushPending(projectId) })
})
...
projects.get('/:id/history/checkpoints/:checkpointId', async (c) => c.json({ checkpoint: await projectHistoryService.getCheckpointSnapshot(id(c), c.req.param('checkpointId')) }))
...
projects.post('/:id/latex/builds', async (c) => {
  ...
  if (typeof input.path === 'string' && input.path) {
    return c.json({ build: await projectLatexService.startFileBuild(id(c), input.path, input.engine) }, 202)
  }
  throw new AppError('PROJECT_LATEX_TARGET_NOT_FOUND', 'targetId or path is required', 400)
})
registerLatexBuildRoutes(projects, '/:id/latex/builds', c => projectLatexService.scoped(c.req.param('id')!))
