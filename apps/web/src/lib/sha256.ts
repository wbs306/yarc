...
const sha256Bytes = (bytes: Uint8Array): string => {
  ...
  for (let offset = 0; offset < padded.length; offset += 64) {
    ...
    for (let index = 16; index < 64; index++) {
      ...
      const next = schedule[index - 2]
      const smallSigma0 = rotateRight(previous, 7) ^ rotateRight(previous, 18) ^ (previous >>> 3)
      const smallSigma1 = rotateRight(next, 17) ^ rotateRight(next, 19) ^ (next >>> 10)
    ...
    for (let index = 0; index < 64; index++) {
      ...
      const temp1 = (h + bigSigma1 + choice + ROUND_CONSTANTS[index] + schedule[index]) >>> 0
...
/**
 * Calculate a SHA-256 text hash in both secure and plain HTTP contexts.
 * SubtleCrypto is unavailable on non-localhost HTTP origins, so keep a
 * compatible synchronous fallback for EasyTier/direct-IP access.
 */
export const sha256Text = async (value: string): Promise<string> => {
  ...
  if (subtle) {
    try {
      ...
      // Fall through to the pure-JavaScript implementation.
    }
  }
