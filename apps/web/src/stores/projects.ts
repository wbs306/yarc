...
export const useProjectsStore = defineStore('projects', () => {
  ...
  async function fetchProjects(archived = false) {
    ...
    error.value = ''
    try {
      const data = await requestJson<{ projects: Project[] }>(`/api/projects${archived ? '?archived=true' : ''}`)
  ...
  async function fetchProject(id: string) {
    const data = await requestJson<{ project: Project }>(`/api/projects/${encodeURIComponent(id)}`)
    currentProject.value = data.project
    return data.project
  }

  async function createProject(input: { name: string; directoryName?: string; description?: string }) {
    const data = await requestJson<{ project: Project }>('/api/projects', { method: 'POST', body: JSON.stringify(input) })
  ...
  async function updateProject(id: string, patch: Partial<Pick<Project, 'name' | 'description' | 'settings'>>) {
    const data = await requestJson<{ project: Project }>(`/api/projects/${id}`, { method: 'PATCH', body: JSON.stringify(patch) })
  ...
  async function archiveProject(id: string) {
    const data = await requestJson<{ project: Project }>(`/api/projects/${id}/archive`, { method: 'POST' })
    projects.value = projects.value.filter(project => project.id !== id)
    return data.project
  }

  async function unarchiveProject(id: string) {
    return (await requestJson<{ project: Project }>(`/api/projects/${id}/unarchive`, { method: 'POST' })).project
  }
  ...
  async function fetchGitStatus(id: string) {
    const data = await requestJson<{ status: ProjectGitStatus }>(`/api/projects/${id}/git/status`)
    gitStatus.value = data.status
    return data.status
  }

  async function fetchLatexTargets(id: string) {
    const data = await requestJson<{ defaultTarget: string | null; targets: ProjectLatexTarget[] }>(`/api/projects/${id}/latex/targets`)
